...
import com.devsync.backend.service.DocumentSnapshotScheduler;
                 ...
                 activeDocuments.forEach(docEntry -> {
                     ...
                     if (parts.length == 2) {
                         ...
                         try {
                            ...
                            // Best-effort Postgres snapshot on leave/disconnect
                            try {
                                documentSnapshotScheduler.flushNow(UUID.fromString(sessionId), documentId);
                            } catch (Exception snapEx) {
                                log.warn("Best-effort snapshot on disconnect failed for session [{}] doc [{}]: {}",
                                        sessionId, documentId, snapEx.getMessage());
                            }
                         } catch (Exception e) {
