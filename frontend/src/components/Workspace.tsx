...
import { useDurableSave } from "../hooks/useDurableSave";
...
const Workspace = ({ room, identity, role, canEdit }: WorkspaceProps) => {
  // REFS
  ...
  const collaborationEnabledMap = useFileStore(
    (state) => state.collaborationEnabled
  );
  const sizeBytesMap = useFileStore((state) => state.sizeBytes);
  ...
  const getPersistenceId = useFileStore((state) => state.getPersistenceId);

  const activeFileCollaborationEnabled = activeFileId
    ? collaborationEnabledMap[activeFileId] !== false
    : true;
  const fileCanEdit = effectiveCanEdit(canEdit, activeFileCollaborationEnabled);
  const activeFileSizeBytes = activeFileId
    ? (sizeBytesMap[activeFileId] ?? 0)
    : 0;
  const isLargeFileReadOnly = Boolean(
    activeFileId && !activeFileCollaborationEnabled
  );
  ...
  const {
    ...
    initialUserColor: identity.color,
    initialSessionId: room.activeSessionId,
  ...
  const { saveStatus } = useDurableSave({
    roomId: room.id,
    canEdit: fileCanEdit,
    enabled: isSessionActive,
  });

  // Lazy-load deferred file content (large / read-only files)
  useEffect(() => {
    if (!activeFileId || !room.id) {
      setContentLoading(false);
      setContentLoadError(null);
      return;
    }

    const existing = useFileStore.getState().fileContents[activeFileId];
    if (existing != null) {
      setContentLoading(false);
      setContentLoadError(null);
      return;
    ...
    const persistenceId = getPersistenceId(activeFileId);
    if (!persistenceId) {
      setContentLoading(false);
      setContentLoadError("Missing file id for content fetch.");
      return;
    }
    ...
    void fetchRoomFileContent(room.id, persistenceId)
      .then((text) => {
        if (cancelled) return;
        setFileContent(activeFileId, text);
        setContentLoading(false);
      })
      .catch(() => {
        if (cancelled) return;
        setContentLoadError("Could not load file content.");
        setContentLoading(false);
      });
  ...
  }, [activeFileId, room.id, getPersistenceId, setFileContent]);
  ...
  // Map room files for explorer + live preview document ids
  const explorerFiles = useMemo(() => {
    const map: {
      [key: string]: { name: string; language: string; content: string };
    } = {};
    (room.files ?? []).forEach((file) => {
      map[file.id] = {
        name: file.path.includes("/")
  ...
  const webViewFileIds = useMemo(() => {
    ...
    return [
      findByPath("index.html"),
      findByPath("style.css"),
      findByPath("script.js"),
  ...
  const handleRunCode = async () => {
    ...
    try {
      ...
      if (!activeFile || contentToRun == null) {
  ...
  // Memos — resolve preview assets by path or normalized OT id
  const resolveFileContent = useCallback(
    (suffix: string, fallback: string) => {
      ...
      if (match) {
        const byId = fileContents[match.id];
        if (byId != null) return byId;
        const byPath = fileContents[match.path];
        if (byPath != null) return byPath;
      }
      const direct = fileContents[suffix];
  ...
  const cssFileContent = useMemo(() => {
    return resolveFileContent("style.css", "/* style.css not loaded */");
  }, [resolveFileContent]);

  const jsFileContent = useMemo(() => {
    return resolveFileContent("script.js", "// script.js not loaded");
  }, [resolveFileContent]);
    ...
    <div className="flex flex-col h-screen bg-workspace text-mist-200 overflow-hidden font-sans">
      ...
      >
        ...
        <MainEditorArea
          ...
          readOnly={!fileCanEdit}
          contentLoading={contentLoading}
          contentLoadError={contentLoadError}
          isLargeFileReadOnly={isLargeFileReadOnly}
          activeFileSizeBytes={activeFileSizeBytes}
        />
      </div>
      {/* Status Bar */}
      <StatusBar
